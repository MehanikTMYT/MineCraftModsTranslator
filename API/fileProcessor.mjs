// fileProcessor.mjs
import { fs, path, tempDir, AdmZip, translate, safeDelete } from './utils.mjs';
import { aiTranslateFunction } from './AI.mjs';
    ...
    for (const langDir of langDirs) {
      ...
      if (fs.existsSync(enUsFile)) {
        ...
        if (options.translateMethod === 'ai') {
          try {
            const jsonContent = JSON.parse(fs.readFileSync(enUsFile, 'utf8'));
            const textToTranslate = JSON.stringify(jsonContent, null, 4);
            const translatedText = await aiTranslateFunction(textToTranslate, options.t);
            translatedDict = JSON.parse(translatedText);
          } catch (err) {
            const errorMessage = `Ошибка при переводе файла с помощью AI: ${err.message}`;
            console.error(errorMessage);
            return { success: false, error: errorMessage };
          }
        } else {
          try {
            translatedDict = await translate(enUsFile, options);
          } catch (err) {
